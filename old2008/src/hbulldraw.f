...
c transpose/invert x and y coordinates of the bullet
         if(rrota(ipl) .eq. 90.) then
           call ngdots(iywin(2,ipl)+iywin(1,ipl)-caygn,caxgn,1,
     &                 size,icolr(ipl))
         elseif(rrota(ipl) .eq. -90.) then
           call ngdots(caygn,ixwin(2,ipl)+ixwin(1,ipl)-caxgn,1,
     &                 size,icolr(ipl))
         elseif(rrota(ipl) .eq. 180. .or. rrota(ipl) .eq. -180.) then
           call ngdots(ixwin(2,ipl)+ixwin(1,ipl)-caxgn,
     &             iywin(2,ipl)+iywin(1,ipl)-caygn,1,size,icolr(ipl))
c MGD end mod
         else
...
c same deal, but for the text
         if(rrota(ipl) .eq. 90.) then
           call plchhq(iywin(2,ipl)+iywin(1,ipl)-caygn,caxgn,
     &       ctitl(ipl)(1:iendct),rtslb(ipl),0.,0.)
         elseif(rrota(ipl) .eq. -90.) then
           call plchhq(caygn,ixwin(2,ipl)+ixwin(1,ipl)-caxgn,
     &       ctitl(ipl)(1:iendct),rtslb(ipl),0.,0.)
         elseif(rrota(ipl) .eq. 180. .or. rrota(ipl) .eq. -180.) then
           call plchhq(ixwin(2,ipl)+ixwin(1,ipl)-caxgn,
     &       iywin(2,ipl)+iywin(1,ipl)-caygn,ctitl(ipl)(1:iendct),
     &       rtslb(ipl),0.,0.)
c MGD end mod
         else
