...
c     write(6,*) 'line rota:', rrota(ipl)
c MGD begin mod
c pretty straight forward transposition/inversion of line 
c end-point coordinated
      if(rrota(ipl) .eq. 90.) then
        call line(iywin(2,ipl)+iywin(1,ipl)-caygn,caxgn,
     &    iywin(2,ipl)+iywin(1,ipl)-cbygn,cbxgn)
      elseif(rrota(ipl) .eq. -90.) then
        call line(caygn,ixwin(2,ipl)+ixwin(1,ipl)-caxgn,cbygn,
     &    ixwin(2,ipl)+ixwin(1,ipl)-cbxgn)
      elseif(rrota(ipl) .eq. 180. .or. rrota(ipl) .eq. -180.) then
        call line(ixwin(2,ipl)+ixwin(1,ipl)-caxgn,iywin(2,ipl)+
     &    iywin(1,ipl)-caygn,ixwin(2,ipl)+ixwin(1,ipl)-cbxgn,
     &    iywin(2,ipl)+iywin(1,ipl)-cbygn)
c MGD end mod
      else
