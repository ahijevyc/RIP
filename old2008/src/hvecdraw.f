...
c
   ...
   90    continue
     ...
     &      prs,ght,ter,sfp,lhidel,idiffflag,cfeld(1,ipl),work1,
     &      pslab1,mabpl,morpl,njx,niy,miy,mjx,mkzh)
         call vinterp(cvcor(ipl),rlevl(ilev,ipl),ixwin(1,ipl),
     &      iywin(1,ipl),icdwk(ipl),vc3d,tmk,qvp,
     &      prs,ght,ter,sfp,lhidel,idiffflag,cfeld(2,ipl),work2,
...
c dimensions...
         call velvctmts(tempu2,morpl,tempv2,morpl,niy,njx,
     &         0.,vmagpmax,1,npaumax,4,vecskip,imxvpl,icolr(ipl))
      endif
c
...
c the one that utilizes the appropriate array (depending on rotation)
  ...
  140    continue
         ...
         call velbrb(pslab1,mabpl,pslab2,mabpl,njx,niy,
  ...
  240    continue
         call gsplci(icolr(ipl))
         call gstxci(icolr(ipl))
         call velbrb(tempu2,morpl,tempv2,morpl,niy,njx,
     &       .4*gskip,4,vecskip,ixwin(1,ipl),iywin(1,ipl),0,rrota(ipl))
         endif
c MGD end mod
         if (.not.lnmsg(ipl)) then
