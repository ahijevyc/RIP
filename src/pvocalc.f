...
c      close(18)
c      read(18)hfx
c      print*,hfx(miy-1,mjx-1)
c      read(18)qfx
c      print*,qfx(miy-1,mjx-1)
c      read(18)pblh
c      print*,pblh(miy-1,mjx-1)
c      close(18)
      icn=0
      do k=1,mkzh
       pstr(k)=0.
...
C
c        pblhav=pblhav + pblh(ii,jj)
c        hfxav=hfxav + hfx(ii,jj)
c        qfxav=qfxav + qfx(ii,jj)
        rhoav=rhoav + dens
...
c       pv(i,j,6)=3.6e3*hfxav*float(iibl)/
c     &      (cp*float(isumt)*rhoav*pblhav)
c       pv(i,j,7)=3.6e3*lv1*qfxav*float(iibl)/
c     &      (cp*float(isumt)*rhoav*pblhav)
c       pv(i,j,7)=3.6e3*qfxav*float(iibl)/
c     &      (cp*float(isumt)*rhoav*pblhav)
C       pv(i,j,8)=10.*float(iibl)/float(isumt)
c       pv(i,j,9)=pv(i,j,5)+pv(i,j,6)+pv(i,j,7)
       pv(i,j,7)=zbmin(i,j)
       pv(i,j,8)=tm
       pv(i,j,12)=tl
...
c
      pavd=50.
      pavd2=25.
      if ((i.eq.100).and.(j.eq.410)) print*,'t=',t
      if ((i.eq.100).and.(j.eq.410)) print*,'p=',p
      if ((i.eq.100).and.(j.eq.410)) print*,'q=',q
      lvl=kk
 10   lvl=lvl+1
      dp=p(lvl)-p(kk)
      if ((i.eq.100).and.(j.eq.410)) print*,'lvl=',lvl,'mkzh=',mkzh
      if ((i.eq.100).and.(j.eq.410)) print*,'dp=',dp,'kk=',kk,'z0=',z0
      ...
      ptop=pbot-pavd
      if ((i.eq.100).and.(j.eq.410)) print*,'ptop=',ptop,'pbot=',pbot
      lvl=lvlbot
 20   lvl=lvl-1
