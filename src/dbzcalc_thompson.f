...
c*********************************************************************c
c                                                                     c
      subroutine dbzcalc(qra,qsn,qgr,tmk,rho,dbz,miy,mjx,mkzh,
...
C..  released as part of WRF-V2.1.  It matches V2.2 (and higher for now).
C..  I would like to bury permanently the code from my 2004 MWR paper -
C..  the new scheme is VERY different from the old.
...
c
     ...
     &     mvd_r(maxk), ilamr(maxk), N0_r(maxk)
      real rs(maxk), rg(maxk), rr(maxk), smob(maxk), smoc(maxk)
      ...
      REAL gonv_max
      PARAMETER (gonv_max = 1.E6)
      REAL ronv_min
      PARAMETER (ronv_min = 2.E6)
      REAL ronv_max
      PARAMETER (ronv_max = 9.E9)
      REAL ronv_sl
      PARAMETER (ronv_sl  = 1./4.)
      REAL ronv_r0
      PARAMETER (ronv_r0 = 0.10E-3)
      REAL ronv_c0
      PARAMETER (ronv_c0 = ronv_sl/ronv_r0)
      REAL ronv_c1
      PARAMETER (ronv_c1 = (ronv_max-ronv_min)*0.5)
      REAL ronv_c2
      PARAMETER (ronv_c2 = (ronv_max+ronv_min)*0.5)
...
C..Various radar related variables
      ...
      REAL D0s, D0g
      PARAMETER (D0s = 125.E-6, D0g = 250.E-6)
...
C..Contribution to reflectivity from rain.
      ...
      melti = .false.
      do k = 2, mkzh
         if ( (temp(k).gt. 273.15) .and. (rr(k).gt. R1)
 ...
 135  continue
      ...
      if (melti) then
C.. Locate bottom of melting layer (if any).
         kbot = mkzh
         do k = k_0+1, mkzh
...
C.. Compute melted snow/graupel equiv water diameter one K-level above
C.. melting.  Set starting rain mvd to either 50 microns or max from
C.. higher up in column.
         xDs = smoc(k_0) / smob(k_0)
         xDg = (bm_g + mu_g + 1.) * ilamg(k_0)
         Ds_m = (am_s*xDs**bm_s / am_r)**obmr
         Dg_m = (am_g*xDg**bm_g / am_r)**obmr
         r_mvd1 = mvd_r(k_0)
         r_mvd2 = MIN(MAX(Ds_m, Dg_m, r_mvd1+1.e-6, SNGL(mvd_r(kbot))),
     &                  3.e-3)

C.. Within melting layer, apply linear increase of rain mvd from r_mvd1
C.. to equiv melted snow/graupel value (r_mvd2).  So, by the bottom of
C.. the melting layer, the rain will have an mvd that matches that from
C.. melted snow and/or graupel.
         if (kbot.lt. mkzh-1) then
         do k = k_0+1, kbot
            xkrat = REAL(k-k_0)/REAL(kbot-k_0)
            mvd_r(k) = MAX(SNGL(mvd_r(k)), xkrat*(r_mvd2-r_mvd1)+r_mvd1)
            lamr = (4.0+mu_r) / mvd_r(k)
            lam_exp = lamr * (crg(3)*org2*org1)**bm_r
            N0_exp = org1*rr(k)/am_r * lam_exp**cre(1)
            N0_exp = DMAX1(DBLE(ronv_min), DMIN1(N0_exp,DBLE(ronv_max)))
            lam_exp = (N0_exp*am_r*crg(1)/rr(k))**ore1
            lamr = lam_exp * (crg(3)*org2*org1)**obmr
            mvd_r(k) = (3.0+mu_r+0.672) / lamr
...
C.. Below melting layer, hold N0_r constant unless changes to mixing
C.. ratio increase mvd beyond 3 mm threshold, then adjust slope and
C.. intercept to cap mvd at 3 mm.  In future, we could lower N0_r to
C.. account for self-collection or other sinks.
         do k = kbot+1, mkzh
            N0_r(k) = DMIN1(N0_r(k), N0_r(kbot))
