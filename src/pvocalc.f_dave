...
c
      ...
      integer ioutc(miy,mjx,mkzh),ioutd(miy,mjx,mkzh),idummy
...
c
c---If fort.19 exists already, assume it is the kbmin3 array.
c
      inquire(file="fort.19",exist=file_exists)
      if (file_exists) then 
        read(19) kbmin3
      else
c
c---Otherwise, initialize new kbmin3 array to be used in future budget calcs (ST)
c
        do i=1,miy
          do j=1,mjx
            do k=1,mkzh
...
C this call to tlift is when bmin (tl) amd bmax (tm) are retrieved.
       ...
       k_to_lift_to = kbmin3(i,j,k)
       call tlift_to_k(t1,dtmk,zmsemax(i,j),p1,q1,dqv,tl,tm,i,j,kk,
     &  mkzh,idummy,k_to_lift_to)
       kk=kmsemax(i,j)
       do 274 ii=ibeg,iend
       do 274 jj=jbeg,jend
...
c       dtmk=pv(i,j,1)
       ...
       dqv=0.
       call tlift_to_k(t1,dtmk,zmsemax(i,j),p1,q1,dqv,tla,tma,
     &     i,j,kk,mkzh,idummy,k_to_lift_to)
...
c       dqv=pv(i,j,4)
       ...
       call tlift_to_k(t1,dtmk,zmsemax(i,j),p1,q1,dqv,tlb,tma,
     &    i,j,kk,mkzh,idummy,k_to_lift_to)
       pv(i,j,11)=tlb-tl
C
