...
c
      write (6,*) njx,niy
c      open(unit=50,file='arlene_nbuoy_shr_init.dat',status='new')
c      ib=780
c      ie=1000
c      if (xtime.eq.36..or.xtime.eq.228.) then
c        jb=110
c        je=130
c      else if (xtime.eq.180.) then
c        jb=4
c        je=224
c      else if (xtime.eq.276.) then
c        jb=30
c        je=250
c      else
c        jb=85
...
c      do i=ib,ie
c        do j=jb,je
c          write (50,999) xtime, i, j, pslab1(i,j)
c        end do
c      end do
c 999  format (1x,f5.1,1x,i4,1x,i4,1x,f10.4)
c
