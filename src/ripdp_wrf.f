...
c
	...
	iend=index(nf_att_text(istart:),' ')-1+istart-1+2
	minfostring='Model Info: '//nf_att_text(istart:iend)  ! First 20 chars.
	minfostring(19:20)='  '    ! get rid of the weird M  jfb
      nf_status = nf_get_att_int (ncid, nf_global,
...
c      nf_status = 333
      if (nf_status .ne. nf_noerr .and. iprog .gt. 2) then
         ...
         if (iprintonce.eq.0) then
            ...
            print*,'This indicates the file is a WRF input file,'
            print*,'rather than output, and pressure perturbation'
            print*,'and base state pressure will have to be calculated'
            print*,'to produce the pressure array.  Proceeding.'
            iprintonce=1
         endif
      else
         igotpressure=1
