...
c
     ...
     &   cin(miy,mjx,mkzh),prsf(miy,mjx,mkzh),mse(miy,mjx,mkzh),
     &   msemax(miy,mjx),zmsemax(miy,mjx),bmin(miy,mjx),zbmin(miy,mjx),
     &   tve(miy,mjx),tlift(miy,mjx),tlc(miy,mjx)
...
c
     ...
     &            exp((thtecon1/tlcl-thtecon2)*q*(1.+thtecon3*q))
               zmse=cp1*t + grav1*ght(i,j,k) + lv1*q
               if ((i.eq.300).and.(j.eq.200)) 
     &     print*,k,zmse/cp1,t,q,ght(i,j,k)
               if (eth.gt.ethmax) then
                  klev=k
                  ethmax=eth
               endif
               if (zmse.gt.msemax1) then
                  klevm=k
                  msemax1=zmse
                  tlc(i,j)=tlcl
               endif
         ...
         kpar1=klevm
         kpar2=klevm
         msemax(i,j)=msemax1/cp1
c         zmsemax(i,j)=ght(i,j,klevm)!-ter(i,j)
         zmsemax(i,j)=ght(i,j,klev)!-ter(i,j)
         kmsemax(i,j)=klev
c         kmsemax(i,j)=klevm
c
...
c
     ...
     &         (tmklift-eslcon2))
            ...
            tvlift=virtual(tmklift,qvplift)
            ghtlift=ght(i,j,k)
            if (tvlift-tvenv.gt.0.) goto 110 !> LFC
         endif
         buoy(kk)=grav*(tvlift-tvenv)/tvenv  ! buoyancy
         if (ght(i,j,k)-ter(i,j).lt.7.e3) then
          if (tvlift-tvenv.lt.bmin(i,j)) then
...
C         cape(i,j,mkzh)=cape(i,j,kpar1)
C         cin(i,j,mkzh)=cin(i,j,kpar1)
C         cape(i,j,mkzh)=msemax(i,j)
C         cin(i,j,mkzh)=zmsemax(i,j)/1.e3
         cape(i,j,mkzh)=msemax(i,j)
         cin(i,j,mkzh)=zmsemax(i,j)/1.e3
         cin(i,j,mkzh-1)=bmin(i,j)
         cin(i,j,mkzh-2)=zbmin(i,j)/1.e3
         kbmin(i,j)=ktemp
         tve(i,j)=tvetemp
         tlift(i,j)=tlifttemp
C         cin(i,j,mkzh-1)=zrel(klcl)+ghtpari-ter(i,j)   ! meters AGL
C         cin(i,j,mkzh-2)=zrel(klfc)+ghtpari-ter(i,j)   ! meters AGL
      endif
c
          if ((i.eq.300).and.(j.eq.200)) print*,kbmin(i,j),tlift(i,j),
     &      tve(i,j)
      enddo
      enddo
      write(16)kbmin
