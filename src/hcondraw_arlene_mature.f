...
c
      write (6,*) njx,niy
      open(unit=50,file='arlene_nbuoy_shr_init.dat',status='new')
      ib=710
      ie=930
c      if (xtime.eq.36..or.xtime.eq.228.) then
      if (xtime.eq.48.) then
        jb=110
        je=330
      else if (xtime.eq.192.) then
        jb=4
        je=224
      else if (xtime.eq.288.) then
        jb=30
        je=250
      else
        jb=85
      ...
      do i=ib,ie
        do j=jb,je
          write (50,999) xtime, i, j, pslab1(i,j)
        end do
      end do
 999  format (1x,f5.1,1x,i3,1x,i3,1x,f10.4)
c
