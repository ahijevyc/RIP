...
c   layer at which the parcel is positively buoyant.
c
      do k=kmax,2,-1
         if (buoy(k).eq.0.) then
      ...
      enddo
c
c   If we got through that loop, then there is no positive buoyancy
c   anywhere in the sounding.  In these regions, both CAPE and CIN will
c   be set to -0.1 J/kg.  Note that where CAPE is non-zero,
c   CAPE and CIN will be set to a minimum of +0.1 J/kg, so that the zero contour
c   in the either the CIN or CAPE fields will circumscribe regions of
c   non-zero CAPE.
c
      cape(i,j,kpar)=-0.1
      cin(i,j,kpar)=-0.1
...
c
 50   continue
c
c   If there is an equilibrium level, then CAPE is positive.
c   We'll define the level of free convection (LFC) as the point
c   below the EL where accumulated buoyant energy is
c   a minimum.  The net positive area (accumulated
c   buoyant energy) from the LFC up to the EL will be
c   defined as the CAPE, and the net negative area (negative of
c   accumulated buoyant energy) from the parcel starting point to the
c   LFC will be defined as the convective inhibition (CIN).
c
c   First get the LFC (i.e. the level below the EL with minimum
c      accum. buoyant energy)
c
      benamin=9e9
      klfc=kmax
      do k=1,kel
