...
c
     ...
     &   sigh(mkzh),rcrag(2,maxpl),rcrbg(2,maxpl),ismth(maxpl),
     &   ixavg(maxpl),icomg(maxpl),rstmv(2,maxpl),idimn(maxpl),
     &   raddf(maxpl),
     &   rmply(maxpl),
     &   ismcp(maxpl),rslcg(2,maxpl),rdiff(maxpl),
...
c
     ...
     &         'xxx.xxx to xxx.xxx'
            if (ifltrack.eq.1) str(iend+1:iend+12)='Flight track'
...
c
      if (field1(10:10).ne.' ') then
         ie1=10
      else
         ie1=index(field1,' ')-1
      endif
      if (field2(10:10).ne.' ') then
         ie2=10
      else
         ie2=index(field2,' ')-1
      endif
      if (field3(10:10).ne.' ') then
         ie3=10
      else
         ie3=index(field3,' ')-1
      endif
...
c
     ...
     &      ' (Added field, factor of ',raddf(ipl),')'
      ...
      if (rmply(ipl).ne.0.0) then
         write(str(39:74),'(a29,f6.2,a1)')
     &      ' (Multiplied field, power of ',rmply(ipl),')'
         goto 300
      endif
c
...
c
     ...
     &    rlevl(ilev,ipl).eq.rlavl(ilev,ipl)) then ! hor. plts, no avg.
         if (cvcor(ipl).eq.'s') then   ! sigma
            write(str(39:57),'(a14,f5.3)')    '   at sigma = ',
     &                    sigh(nint(rlevl(ilev,ipl)))
     ...
     &           cvcor(ipl).eq.'x') then   ! pressure
            if (nint(rlevl(ilev,ipl)).eq.1001) then
               write(str(39:60),'(a15)')   'at true surface'
            else
     ...
     &    rlavl(ilev,ipl).ge.0.) then  ! hor. plts w/ avg.
         if (cvcor(ipl).eq.'s') then   !sigma
            write(str(39:63),'(a11,f5.3,a4,f5.3)')    'Avg, sig = ',
     &           sigh(nint(rlevl(ilev,ipl))),' to ',
     &           sigh(nint(rlavl(ilev,ipl)))
...
c     &        nint(rlevl(ilev,ipl)),' to ',nint(rlavl(ilev,ipl)),' K '
      ...
      elseif (cptyp(ipl)(1:1).eq.'h'.and.
     &    rlevl(ilev,ipl).ne.rlavl(ilev,ipl).and.
     &    rlavl(ilev,ipl).lt.0.) then  ! hor. plts w/ sigma-level differencing
         if (cvcor(ipl).eq.'s') then   !sigma
            write(str(39:69),'(a16,f5.3,a5,f5.3)')  'Diff betw sig = ',
     &           sigh(nint(rlevl(ilev,ipl))),' and ',
     &           sigh(nint(-rlavl(ilev,ipl)))
         else
            write(iup,*) 'RIP is only set up to do averaging when'
            write(iup,*) 'the vertical coordinate is sigma.'
            stop
         endif
