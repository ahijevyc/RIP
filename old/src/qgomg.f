...
c*********************************************************************c
c                                                                     c
      subroutine qgomg(prs,omg,tmk,qvp,ght,bvfsqd,bvfsqm,rhi,cor,xmap,
     &   ter,numpas,ivar,ivar2,iqvecforc,itopobc,iekmnbc,imo,rhithresh,
...
c   the following one:
c
c      subroutine qgomg(prs,omg,tmk,qvp,ght,bvfsqd,bvfsqm,rhi,cor,xmap,ter,
c     &   numpas,ivar,ivar2,iqvecforc,itopobc,iekmnbc,imo,rhithresh,
c     &   fname,iendf1,ihrip,rhrip,fullsigma,halfsigma,
c     &   chrip,vardesc,plchun,miydumb,mjxdumb,mkzhdumb)
c
...
c
      do kp=1,mkp
         plev(kp)=ptopqg+(kp-1.)*dp
      enddo
c
c   Try to get omg, ght, alpha, stbld, stblm, and rhi
c   on pressure levels.
c
      write(temp,'(i4)')1000+numpas
c
      varname='omgqg'//temp(2:4)
      call readdat(iudata,fname,iendf1,varname,
     &      miy,mjx,mkp,3,0,omgqg,istatomgqg)
      varname='ghtqg'//temp(2:4)
      call readdat(iudata,fname,iendf1,varname,
     &      miy,mjx,mkp,3,0,ghtqg,istatghtqg)
      varname='alphaqg'//temp(2:4)
      call readdat(iudata,fname,iendf1,varname,
     &      miy,mjx,mkp,3,0,alphaqg,istatalphaqg)
      varname='stbldqg'//temp(2:4)
      call readdat(iudata,fname,iendf1,varname,
     &      miy,mjx,mkp,3,0,stbldqg,istatstbldqg)
      if (imo.eq.1) then
         varname='stblmqg'//temp(2:4)
         call readdat(iudata,fname,iendf1,varname,
     &         miy,mjx,mkp,3,0,stblmqg,istatstblmqg)
         varname='rhiqg'//temp(2:4)
         call readdat(iudata,fname,iendf1,varname,
     &         miy,mjx,mkp,3,0,rhiqg,istatrhiqg)
      else
         istatstblmqg=1
         istatrhiqg=1
      endif
      ...
      call readdat(iudata,fname,iendf1,varname,
     &      miy,mjx,mkp,2,0,terqg,istatterqg)
c
...
c
c   If some files did not exist, calculate them.
c
      do kp=1,mkp
      do j = 1, mjx-1
      do i = 1, miy-1
         if (plev(kp).gt.prs(i,j,mkzh)) then
            ipttp(i,j,kp)=-1  ! point is below ground (missing data)
         ...
         elseif (plev(kp).lt.prs(i,j,1)) then
            write(iup,*)'At point i,j,kp=',i,j,kp,
     &         '  pressure is too low.'
            write(iup,*)'plev,prs=',plev(kp),prs(i,j,1)
            stop
         else
            ipttp(i,j,kp)=0  ! point is within domain
            do ks=1,mkzh-1
               if (plev(kp).le.prs(i,j,ks+1).and.
     &             plev(kp).ge.prs(i,j,ks)) then
                  fac1=plev(kp)-prs(i,j,ks)
                  fac2=prs(i,j,ks+1)-plev(kp)
...
c         if (plev(kp).gt.pmaxallowed) omgqg(i,j,kp)=rmsg
c
...
c
      do i=1,32
         ihripp(i)=ihrip(i)
      enddo
      ihripp(8)=2+ihripp(8)   ! 2/3  means hyd./nonhyd. prs-lev. data
      do k=1,mkp
         plevels(k)=plev(k)
      enddo
      do k=mkp+1,128
         plevels(k)=halfsigma(128)
      enddo
      if (istatomgqg.eq.-1) then
     ...
     &      3,1,vardesc,plchun,fname,iendf1,
     &      ihripp,rhrip,chrip,fullsigma,plevels,miy,mjx,mkp)
      endif
      if (istatghtqg.eq.-1) then
...
c
      if (istatterqg.eq.-1) then
         ...
         if (numpas.gt.0) then
            ...
            call smooth(terqg,numpas,miy,miy-1,mjx-1)
         endif
         vardesc='Smoothed terrain height'
         plchun='m'
         varname='terqg     '
         call writefile(terqg,varname,numpas,
     &      2,1,vardesc,plchun,fname,iendf1,
     &      ihripp,rhrip,chrip,fullsigma,plevels,miy,mjx,mkp)
      endif
c
...
c      numpstb=0
     ...
     &          rhiqg(i,j,k).gt.rhithresh) then
      ...
      enddo
         if (imo.eq.1) then
            call smooth(stblqg(1,1,k),numpstb,miy,miy-1,mjx-1)
         endif
      enddo
c
...
c                  print*,'k,plev,grav,fbar,d2zdy2,d2zdxdy,dadx,dady='
c                  print*,k,plev(k),grav,fbar,d2zdy2,d2zdxdy,dadx,dady
c               endif
            else
...
c
     ...
     &             rhiqg(i,j,k).gt.rhithresh) then
         ...
         enddo
         enddo
            call smooth(fac(1,1,k),numpstb,miy,miy-1,mjx-1)
            do j=1,mjx-1
            do i=1,miy-1
...
c         ihripp(i)=ihrip(i)
c      enddo
c      ihripp(8)=2+ihripp(8)   ! 2/3  means hyd./nonhyd. prs-lev. data
c      do k=1,mkp
c         plevels(k)=plev(k)
c      enddo
c      do k=mkp+1,128
c         plevels(k)=halfsigma(128)
c      enddo
...
c     &   3,1,vardesc,plchun,fname,iendf1,
c     &   ihripp,rhrip,chrip,fullsigma,plevels,miy,mjx,mkp)
c
c   Interpolate back to sigma.
c
      do j = 1, mjx-1
      do i = 1, miy-1
c
     ...
     &       (ivar.lt.4.and.ipttp(i,j,kp).eq.1)) then
      ...
      if (kvmax-kvmin.lt.1) then
         ...
         do ks=1,mkzh
            if (prs(i,j,ks).le.plev(kvmax).and.
     &          prs(i,j,ks).ge.plev(kvmin)) then
               do kp=kvmin,kvmax-1
                  if (prs(i,j,ks).le.plev(kp+1).and.
     &                prs(i,j,ks).ge.plev(kp)) then
                     omg(i,j,ks)=((prs(i,j,ks)-plev(kp))*
     &                   omgqg(i,j,kp+1)+(plev(kp+1)-prs(i,j,ks))*
     &                   omgqg(i,j,kp))*dpi
                     goto 50
                  endif
               enddo
 50            continue
            elseif (prs(i,j,ks).gt.plev(kvmax)) then
               omg(i,j,ks)=omgqg(i,j,kvmax)
            elseif (prs(i,j,ks).lt.plev(kvmin)) then
