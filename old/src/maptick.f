...
c   routine.  This routine calls the routine maptform, converts
c   MM5 x/y coordinates to lat/lon, and vice versa.
c
c   Input:
...
c
      do i=1,nres
         yy(i)=yllc+(i-1.)*ddy
         xx(i)=xllc+(i-1.)*ddx
         call maptform(yy(i),xllc,rlatleft(i),rlonleft(i),1,rrota)
         call maptform(yy(i),xurc,rlatright(i),rlonright(i),1,rrota)
         call maptform(yllc,xx(i),rlatbot(i),rlonbot(i),1,rrota)
         call maptform(yurc,xx(i),rlattop(i),rlontop(i),1,rrota)
