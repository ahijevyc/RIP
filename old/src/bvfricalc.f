...
c*********************************************************************c
c                                                                     c
      subroutine bvfricalc(sigh,pstx,prs,tmk,velscr,qvp,qls,
     &   uvel,vvel,cosa,sina,cphase,field,isat,arr,miy,mjx,mkzh)
c
...
c
     ...
     &   vvel(miy,mjx,mkzh),arr(miy,mjx,mkzh)
      character field*10
c
      dimension ws(100),wt(100),th(100),rh(100),dsdz(100)
c
      include 'comconst'
c
      if (field(1:2).ne.'bv') then
         if (field(7:9).ne.'   ') then
            read(field(7:9),'(i3)') nkmsmth
            numpas=300+nint(nkmsmth/dskm)
...
c         Smooth this level
c
            call smooth(velscr(1,1,k),numpas,miy,miy-1,mjx-1)
         enddo
      endif
c
      do 200 j=1,mjx-1
...
c
     ...
     &         log(refprspas/1.e5)))
            dsdz(k)=-refrho*grav/(pstx(i,j)*100.)
         else
            dsdz(k)=-(sigh(k)+ptop/pstx(i,j))*grav/(rgas*
...
c
     ...
     &         ((th(kp1)-th(km1))/(sigh(kp1)-sigh(km1))*dsdz(k)/th(k)+
     &         xlhc/(cp*tmk(i,j,k))*(ws(kp1)-ws(km1))/
     &         (sigh(kp1)-sigh(km1))*dsdz(k))-(wt(kp1)-wt(km1))/
     &         (sigh(kp1)-sigh(km1))*dsdz(k))
         else
            bvfsq=grav*(th(kp1)-th(km1))/(sigh(kp1)-sigh(km1))*dsdz(k)/
     &         th(k)
         endif
...
c
     ...
     &              (sigh(kk+1)-sigh(kk))*.5*(dsdz(kk+1)+dsdz(kk))-
     &              (velscr(i,j,kk)-velscr(i,j,kk-1))/
     &              (sigh(kk)-sigh(kk-1))*.5*(dsdz(kk)+dsdz(kk-1)))/
     &             (.5*(sigh(kk+1)-sigh(kk-1)))*dsdz(kk)
