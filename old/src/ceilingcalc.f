...
c*********************************************************************c
c                                                                     c
      subroutine ceilingcalc(qvp,qcw,qra,qci,qsn,tmk,sigh,sigf,pstx,prs,
...
c
      exting=1.
      clg(i,j)=0.
      pup=prs(i,j,mkzh)+(1.-sigh(mkzh))*pstx(i,j)
      do 150 k=mkzh,1,-1
         wmix=qvp(i,j,k)
         if (iice.eq.0) then
            qprc=qra(i,j,k)
     ...
     &        + 1.e-10
         pdn=pup
         if (k.eq.1) then
            pup=prs(i,j,1)-sigh(1)*pstx(i,j)
         else
            pup=((sigh(k)-sigf(k))*prs(i,j,k-1)+
