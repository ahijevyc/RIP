...
c*********************************************************************c
c                                                                     c
      subroutine vwinddraw(ilinw,sigh,rcrag,rcrbg,ismth,unwk,icomg,
     &         icolr,rvcmx,cfulb,rvvms,ivvnx,ixavg,work1,work2,icdwk,
...
c
      dimension sigh(mkzh),rcrag(2,maxpl),rcrbg(2,maxpl),
     ...
     &   unorth(miy,mjx),vnorth(miy,mjx),rrota(ipl)
      logical lnmsg(maxpl)
...
c
      ...
      call smooth(pslab2,ismth(ipl),mabpl,nscrs,mkzh)
c
...
c
      ...
      dsigh=0.
      do 125 k=mkzh-1,1,-1
         dsigh=dsigh+sigh(k+1)-sigh(k)
         if (dsigh.gt.dsigmin) then
            dsigh=0.
            idolev(k)=1
         else
            idolev(k)=0
...
c
  ...
  140    continue
         ...
         ivcs=1
         call velbrb(pslab1,mabpl,pslab2,mabpl,nscrs,mkzh,
     &            .4*gskip,4,vecskip,1,1,1,rrota(ipl))
         ivcs=0
         if (.not.lnmsg(ipl)) then
            call setusv('LW',1000)
            call set(0.,1.,0.,1.,0.,1.,0.,1.,1)
